...
class AdvancedBIModel {
  ...
  static async getRevenueIntelligence(salonId, startDate, endDate) {
    try {
      ...
      // Get Staff-wise Revenue (Performance analysis) - simplified
      const [staffRevenue] = await pool.query(
        `SELECT
          s.id,
          s.name as staff_name,
          0 as bookings_completed,
    ...
    try {
      const [serviceMetrics] = await pool.query(
        `SELECT
          ...
          COALESCE(AVG(bi.price), 0) as avg_price,
          ROUND((COUNT(bi.id) / (SELECT COUNT(*) FROM booking_items WHERE booking_id IN (SELECT id FROM bookings WHERE salon_id = ? AND DATE(booking_date) BETWEEN ? AND ?))) * 100, 2) as market_share_pct
        FROM services s
        LEFT JOIN categories c ON s.category_id = c.id
        LEFT JOIN booking_items bi ON s.id = bi.service_id
        LEFT JOIN bookings b ON bi.booking_id = b.id
      ...
      return {
        summary: {
          ...
          total_service_revenue: serviceMetrics.reduce((sum, s) => sum + parseFloat(s.revenue || 0), 0).toFixed(2),
          avg_service_price: serviceMetrics.reduce((sum, s) => sum + parseFloat(s.avg_price || 0), 0) / serviceMetrics.length
        },
        service_details: serviceMetrics,
        chart_data: {
          service_revenue: serviceMetrics.map(s => ({ name: s.name, revenue: parseFloat(s.revenue) })),
          service_bookings: serviceMetrics.map(s => ({ name: s.name, bookings: s.bookings }))
        }
      };
    } catch (error) {
