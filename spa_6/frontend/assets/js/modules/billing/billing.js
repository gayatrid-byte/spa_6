...
async function showInvoiceForm(invoice = null) {
  ...
  window.billingModule.loadServiceData = async function (customerId, date) {
    try {
      ...
      const bookings = await api.bookings.getAll(bookingFilters);
      ...
      if (bookings && bookings.length > 0) {
        ...
        console.log('First booking total_amount:', bookings[0].total_amount);
        console.log('First booking subtotal_amount:', bookings[0].subtotal_amount);
      }
      ...
      if (bookings && bookings.length > 0) {
        ...
        // Extract all booking IDs from invoices (stored at invoice level, not item level)
        customerInvoices.forEach(inv => {
          console.log('Processing invoice:', inv.invoice_number, 'booking_ids:', inv.booking_ids);
          
          // Check if invoice has booking_ids field
          if (inv.booking_ids) {
            let ids;
            if (typeof inv.booking_ids === 'string') {
              try {
                ids = JSON.parse(inv.booking_ids);
              } catch (e) {
                console.error('Failed to parse booking_ids JSON:', inv.booking_ids);
                ids = [];
              }
            } else if (Array.isArray(inv.booking_ids)) {
              ids = inv.booking_ids;
            } else {
              ids = [];
            }
            ...
            console.log('  No booking_ids in invoice');
          }
        });
        
        console.log('Invoiced booking IDs:', Array.from(invoicedBookingIds));
        
        // Filter to only include bookings without invoices (ensure both sides are numbers for comparison)
        availableBookings = bookings.filter(b => {
          const bookingId = Number(b.id);
          const isAlreadyInvoiced = invoicedBookingIds.has(bookingId);
          console.log(`Checking booking ${bookingId}: already invoiced = ${isAlreadyInvoiced}`);
      ...
      if (availableBookings.length === 0) {
        ...
        // Update the calculation summary to reflect no services
        window.updateInvoiceCalculations();
        ...
        try {
          ...
          // If no booking_totals from API, use the original booking data we already have
          let bookingSubtotalSum, bookingDiscountSum, bookingTaxSum, bookingWalletSum, bookingTotalSum;
          
          if (filteredTotals.length > 0) {
            // Use API booking_totals if available
            bookingSubtotalSum = filteredTotals.reduce((s, t) => s + (parseFloat(t.subtotal_amount) || 0), 0);
            bookingDiscountSum = filteredTotals.reduce((s, t) => s + (parseFloat(t.discount_amount) || 0), 0);
            bookingTaxSum = filteredTotals.reduce((s, t) => s + (parseFloat(t.tax_amount) || 0), 0);
            bookingWalletSum = filteredTotals.reduce((s, t) => s + (parseFloat(t.wallet_applied) || 0), 0);
            bookingTotalSum = filteredTotals.reduce((s, t) => s + (parseFloat(t.total_amount) || 0), 0);
          } else {
            // Use original booking data we already have
            const availableBookingData = availableBookings.filter(b => bookingIdSet.has(Number(b.id)));
            bookingSubtotalSum = availableBookingData.reduce((s, b) => s + (parseFloat(b.subtotal_amount) || 0), 0);
            bookingDiscountSum = availableBookingData.reduce((s, b) => s + (parseFloat(b.discount_amount) || 0), 0); 
            bookingTaxSum = availableBookingData.reduce((s, b) => s + (parseFloat(b.tax_amount) || 0), 0);
            bookingWalletSum = availableBookingData.reduce((s, b) => s + (parseFloat(b.wallet_applied) || 0), 0);
            bookingTotalSum = availableBookingData.reduce((s, b) => s + (parseFloat(b.total_amount) || 0), 0);
          }
          ...
          // Update servicesActualSubtotal from booking totals, otherwise use fallback
          if (bookingSubtotalSum > 0) {
